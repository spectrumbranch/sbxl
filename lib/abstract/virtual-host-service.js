			...
			return qfs.stat(self.sitesAvailableDir + siteAvailable.name).then(function(virtualHost) {
				...
				if (err.code == 'ENOENT') {
					...
					return self.writeSiteAvailable(self.sitesAvailableDir + siteAvailable.name, siteAvailable.contents).then(function() {
						return self.enableSite(siteAvailable.name, self.sitesAvailableDir + siteAvailable.name, self.sitesEnabledDir + siteAvailable.name);
					});
				} else {
					console.log('Unrecognized error: ', err);
					throw err;
...
vhs.prototype.enableSite = function(site, siteAvailablePath, siteEnabledPath) {
	var self = this;
	return self.enableSiteFn(site, siteAvailablePath, siteEnabledPath);
}
