		...
		return q.all(storageData.map(function(siteAvailable) {
			...
			return qfs.stat(self.sitesAvailableDir + siteAvailable.name).then(function(virtualHost) {
				...
				if (err.code == 'ENOENT') {
					...
					return qfs.write(self.sitesAvailableDir + siteAvailable.name, siteAvailable.contents).then(function() {
						//written file
						return;//
					}).catch(function(writeError) {
						if (writeError.code == 'EACCES') {
							//Need to quit and tell the user they need sudo.
							console.log('This process may require permissions in order to write file: ', self.sitesAvailableDir + siteAvailable.name);
							return;
						} else {
